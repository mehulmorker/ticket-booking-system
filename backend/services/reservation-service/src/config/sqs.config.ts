...
export default registerAs("sqs", () => {
  const reservationExpiryQueueUrl =
    process.env.RESERVATION_EXPIRY_QUEUE_URL ||
    "http://localhost:4566/000000000000/reservation-expiry-queue";

  // Detect if using AWS (queue URL contains amazonaws.com) or LocalStack
  const isAWS = reservationExpiryQueueUrl.includes("amazonaws.com");
